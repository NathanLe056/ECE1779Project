...
export const validateTournamentDescription = (
  req: Request,
  res: Response,
  next: NextFunction
) => {
...
export const validateBracketSize = (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  ...
  if (!Number.isInteger(bracket_size)) {
    return res.status(400).json({
      message: "bracket_size must be an integer",
    });
  }

  if (bracket_size !== 6) {
    return res.status(400).json({
      message: "bracket_size must be exactly 6 for the 6-player bracket format",
    });
  }
